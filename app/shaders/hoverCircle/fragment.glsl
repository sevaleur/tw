...
void main()
{
  ...
  float dist = 1.0 - distance(vPosition.xy, vec2(0.0));
  float delta = fwidth(dist);
  float alpha = uAlpha - smoothstep(0.6 - delta, 0.5, dist);

  vec4 noise = texture2D(tDisplacement, vUv + (uTime * 0.02)) * alpha;
  float state = uState * 0.66 + noise.g * 0.04;

  float circ = 1.0 - smoothstep(-uWidth, 0.0, uRadius * distance(center * ratio, vUv * ratio) - state * ( 1. + uWidth ));
