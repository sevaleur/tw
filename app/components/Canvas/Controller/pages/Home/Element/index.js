...
{
  constructor({ element, index, hoverImage, template, link, geometry, scene, screen, viewport })
  {
  ...
  createTexture()
  {
    ...
    let hoverSrc = this.hoverImage.getAttribute('data-src')

    this.texture = window.IMAGE_TEXTURES[src]
    this.secText = window.IMAGE_TEXTURES[hoverSrc]

    this.material.uniforms.tMap.value = this.texture 
    this.material.uniforms.u_hoverImage.value = this.secText 
